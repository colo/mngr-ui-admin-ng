...
let debug = require('debug')('mngr-ui-admin:apps:graphql:pipelines:os.resolvers')
...
module.exports = function(query, app){
	let resolvers = {
		...
		Query: {
			// os: async (obj, args, context, info) => {
			...
			// 			query = query
			// 				.filter(app.r.row('metadata')('host').eq(args.id))
			//
			// 		query = query
			// 			.withFields({metadata: 'path'})
			// 			.distinct()
			//
			// 		// cursor = await query('metadata')('path').distinct()
			...
			// 		.orderBy({index: app.r.desc('timestamp')})
			// 		.filter(
			// 			app.r.row('metadata')('host').eq(args.id).and(
			...
			os: async (obj, args, context, info) => {
				...
				if(args.id && args.id !== undefined){
					...
					if(args.path !== undefined)
						filters.push(app.r.row('metadata')('path').eq('os.'+args.path))
