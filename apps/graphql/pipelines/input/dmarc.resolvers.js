...
let debug = require('debug')('mngr-ui-admin:apps:graphql:pipelines:munin.resolvers')

module.exports = function(query, app, range){
	let resolvers = {
		Query: {
			dmarc: async (obj, args, context, info) => {
				...
				// 	let disp = {'data': {'records': {}}}
				// 	disp.data.records[args.disposition] = true
				//
				// 	// query = query.hasFields(disp)
				// 	fields.push(disp)
				// 	debug('disposition %O ', disp)
				// 	// process.exit(1)
				// }
				...
				if(args.id && args.id !== undefined){
					...
					if(args.order !== undefined && args.order === 'asc'){
						query = query
						.orderBy({index: app.r.asc('timestamp')})
					}
					else{
						query = query
					...
					if(args.path !== undefined)
						filters.push(app.r.row('metadata')('path').eq('dmarc.'+args.path))
				...
				// 	debug('cursor.toArray()', result, result.length)
				// 	process.exit(1)
				// })
