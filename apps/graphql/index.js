...
let debug = require('debug')('mngr-ui-admin:apps:graphql'),
    debug_internals = require('debug')('mngr-ui-admin:apps:graphql:Internals');
...
module.exports = new Class({
    ...
    api: {
      path: '',
		...
		io: {
			...
			// rooms: ['graphql'], //atomatically join connected sockets to this rooms
			routes: {
        '/': [{
					// path: ':param',
					// once: true, //socket.once
					callbacks: ['graphql', 'register'],
        ...
        if(format !== undefined){
          if(opts.query.index !== false){//data get grouped onto arrays
            eachOf(result.data, function (grouped_value, grouped_key, to_grouped_output) {
              ...
              // process.exit(1)
              this.generic_response({err, result, resp: undefined, socket, input: 'graphql', opts})
            }.bind(this));
          }
          else{
  ...
  graphql: function(){
    ...
    if(opts.query && opts.query.register && socket){
      ...
      debug_internals('graphql: %o %o %o', opts.params, opts.query, opts.body)
			// process.exit(1)

      // debug_internals('hosts: %o %o %o', arguments)
      if(opts.body && opts.query)
        opts.query = Object.merge(opts.query, opts.body)
      ...
      **/
      ...
      //       // debug('PRE OUTPUT %o', responses)
      //       // process.exit(1)
      //       this.generic_response({err, result, resp, socket, input: 'graphql', opts})
      //     }.bind(this));
      //   }
      //   else{
      //     this.data_formater(result.data, format, data_formater_full, function(data){
