...
module.exports = new Class({
        ...
        if(format !== undefined){
          if(opts.query.index !== false){//data get grouped onto arrays
            eachOf(result.data, function (grouped_value, grouped_key, to_grouped_output) {
              ...
              // process.exit(1)
              this.generic_response({err, result, resp: undefined, socket, input: 'hosts', opts})
            }.bind(this));
          }
          else{
  ...
  hosts: function(){
    ...
    if(opts.query && opts.query.register && socket){
      ...
      debug_internals('hosts: %o %o %o', opts.params, opts.query, opts.body)
			// process.exit(1)
      ...
      from = (Array.isArray(from)) ? from : [from]
        ...
        if(opts.query.index !== false){//data get grouped onto arrays
          eachOf(result.data, function (grouped_value, grouped_key, to_grouped_output) {
            ...
            // debug('PRE OUTPUT %o', responses)
            // process.exit(1)
            this.generic_response({err, result, resp, socket, input: 'hosts', opts})
          }.bind(this));
        }
        else{
          this.data_formater(result.data, format, data_formater_full, function(data){
