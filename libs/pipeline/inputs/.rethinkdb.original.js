      ...
      once: [
        {
            ...
            if(!req.query || (!req.query.register && !req.query.unregister)){
              ...
              /**
              * orderBy need to be called before filters (its order table), other transform like "slice" are run after "filters"
              **/
              let transformation = (req.query && req.query.transformation) ? req.query.transformation : undefined
              if(
                transformation
                && (transformation.orderBy
                  || (Array.isArray(transformation) && transformation.some(function(transform){ return Object.keys(transform)[0] === 'orderBy'}))
                )
              ){
                let orderBy = (transformation.orderBy) ? transformation.orderBy : transformation.filter(function(transform){ return Object.keys(transform)[0] === 'orderBy' })[0]//one orderBy
                query = app.query_with_transformation(query, orderBy)

                if(Array.isArray(transformation)){
                  transformation = Array.clone(transformation)
                  transformation.each(function(transform, index){
                    if(Object.keys(transform)[0] === 'orderBy')
